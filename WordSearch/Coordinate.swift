...
struct Coordinate {
    ...
    static func decodeCsv(_ csv: String) -> [Coordinate]? {
    ...
    func isCardinalToCoordinate(_ coordinate: Coordinate) -> Bool {
        let deltaX = self.x - coordinate.x
        let deltaY = self.y - coordinate.y
        let radians = atan2(Double(deltaX), Double(deltaY))
        let degrees = abs(radians * (180.0 / .pi)).truncatingRemainder(dividingBy: 90)
